...
| 壁纸库视图 | 点「选一张图…」之后的浏览器视图 |
|---|---|
| ![选择器](template/docs/picker.png) | ![选择器-浏览](template/docs/picker-browse.png) |
> 📷 上面两张图截自**更早的版本**：窗口标题当时还是英文，主按钮当时叫「浏览文件夹…」。
> 现在按钮叫「选一张图…」，点它是**在界面里**翻图 —— 不再弹 Windows 那个看不见缩略图的
> 选文件夹框。界面主体（缩略图网格、状态栏、档位按钮）与截图一致。

- Windows 原生（`mshta.exe`），**不依赖 node** —— node 坏了界面照样能开
- 带缩略图，一屏看清「已在壁纸库」和「正在使用」
- 浏览模式只列**预览**；点中哪张，才真收哪张进去（不会把整个文件夹全收进来）
- 选图用的是**界面自己的**浏览器（缩略图网格），不弹 Windows 那个看不见缩略图的选文件夹框
