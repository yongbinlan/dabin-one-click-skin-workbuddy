...
| 壁纸库视图 | 点「在界面里翻…」之后的浏览器视图 |
|---|---|
| ![选择器](template/docs/picker.png) | ![选择器-浏览](template/docs/picker-browse.png) |
> 📷 上面两张图截自**更早的版本**：窗口标题当时还是英文，按钮行也不一样。现在主按钮叫
> 「用资源管理器选…」—— 点它弹出的是**资源管理器自己的选图框**（有缩略图、有左侧导航、
> 能搜索），旁边还有一个「在界面里翻…」走界面自带的缩略图浏览器。
> 界面主体（缩略图网格、状态栏、档位按钮）与截图一致。

- Windows 原生（`mshta.exe`），**不依赖 node** —— node 坏了界面照样能开
- 带缩略图，一屏看清「已在壁纸库」和「正在使用」
- 浏览模式只列**预览**；点中哪张，才真收哪张进去（不会把整个文件夹全收进来）
- 选图两条路并列：**「用资源管理器选…」**（系统那个通用对话框 —— 缩略图 / 左侧导航 /
  搜索 / 最近位置都有）与 **「在界面里翻…」**（界面自带的缩略图浏览器）。
  前者更顺手，后者是它的兜底 —— 前者被安全策略拦住时，它照样挑得到图
- 翻到哪一层，那层的**文件夹和图片一起列出来**；状态行给「上一级 / 换个位置 / 返回壁纸库」
- 产物是**纯 ASCII**（中文以 `\uXXXX` 转义存在）—— 从根上绕开 MSHTML 的编码坑
- 起点**不会**记成「壁纸库目录」自己；已经在这一层翻的时候再点它 = 回到「换个位置」那一屏
  —— 免得停在和壁纸库长得一模一样的目录里，让人以为按钮坏了（2026-09-18 修）
- `last-dir` 的读写两端都用 ANSI，路径含中文时"上次挑图的位置"不会失效
