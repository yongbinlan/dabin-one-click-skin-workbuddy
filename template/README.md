...
现在改为**界面自己的图片浏览器**：点「选一张图…」后，当前这一层的文件夹和图片一起铺成
缩略图网格，点文件夹进去、点图片换上；状态行给「上一级 / 换个位置 / 返回壁纸库」，
并记住上次挑图的位置。全程不依赖任何系统对话框。

系统对话框没删，退成备用入口（位置列表那一屏的「用系统对话框选…」）——
网络位置、库这类地方它确实更顺手。

**它有两个会让人以为「按钮坏了」的坑（2026-09-18 修）**：

1. 起点记忆原先会把**壁纸库目录自己**记成"上次挑图的地方"（旧版自检写的，
   而且自检不还原）→ 点「选一张图…」跳进去，看到的图和壁纸库视图**一模一样**；
